...
void sidesofpyramid() {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        glVertex3f((float)(2*cos(i * 2 * M_PI / n)), 0, (float)(2 * sin(i * 2 * M_PI / n)));
        glVertex3f((float)(2 * cos((i + 1) * 2 * M_PI / n) ), 0, (float)(2 * sin((i + 1) * 2 * M_PI / n)));
    }
    glEnd();
}
...
void polygon() {
    ...
    glVertex3f(0, 0, 0);
    for (float i = 0; i < 8; i++) {
        x = ((float)(2 * cos(i * 2.0 * M_PI / 7)));
        z = (float)(2 * sin(i * 2.0 * M_PI / 7));
       glVertex3f(x, 0, z);
    }
    glEnd();
}
 ...
 void Cork() {
     ...
     int numVertices = 6000;
     double angleInc = 60 * M_PI / 30000;
     float height = -3;
     float width =1.0f;
     ...
     for (int i = 1; i <= numVertices; i++) {
         width =9.0f*i/numVertices;
         glPointSize(width);
         glBegin(GL_POINTS);
         glColor3f(x / 0.6, z*x / 0.4, z / 0.5);
         x = (float)(cos(angle));
         z = (float)(sin(angle));
     ...
     glTranslated(0.2, height+0.75, -4);
     glColor3f(0.5, 0.5, 0.5);
     gluCylinder(gluNewQuadric(), 1, 1, 8, 50, 10);
}

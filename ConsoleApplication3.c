...
void shape() {
    ...
    for (int i = 0; i < n; i++)
    {
        glMaterialfv(GL_FRONT,GL_DIFFUSE, materials[i]);
        glVertex3f((float)(2 * cos(i * 2 * M_PI / n)), -1, (float)(2 * sin(i * 2 * M_PI / n)));
        glMaterialfv(GL_FRONT, GL_AMBIENT, materials[i]);
        glVertex3f((float)(2 * cos((i + 1) * 2 * M_PI / n)), -1, (float)(2 * sin((i + 1) * 2 * M_PI / n)));
    }
    glEnd();
}
void display() {
...
void initGL() {
    ...
    float tab[4] = { 0.8, 0.8, 0.8,1 };
    float pos[4] = { 1,3,1,1 };
    glLightfv(GL_LIGHT1, GL_DIFFUSE, tab);
